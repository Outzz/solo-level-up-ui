      ...
      <motion.div layout className="flex items-center gap-3 p-4 rounded-lg border bg-card border-primary/40">
        ...
        <div className="flex-1 space-y-2">
          <input
            ...
            className="w-full bg-input border border-border rounded px-3 py-1.5 text-foreground font-body focus:outline-none focus:ring-2 focus:ring-primary text-sm"
          />
          <input
            type="number"
            value={editXp}
            onChange={e => setEditXp(Number(e.target.value))}
            className="w-20 bg-input border border-border rounded px-3 py-1.5 text-foreground font-body focus:outline-none focus:ring-2 focus:ring-primary text-sm"
          />
        </div>
    ...
    >
      ...
      <div className="flex items-center gap-1">
        {!mission.completed && onEdit && (
          <button onClick={() => setEditing(true)} className="p-2 text-muted-foreground hover:text-neon-blue hover:bg-secondary rounded-lg transition-colors">
            <Pencil size={16} />
          </button>
        )}
        {onDelete && (
          <button onClick={() => onDelete(mission.id)} className="p-2 text-muted-foreground hover:text-destructive hover:bg-secondary rounded-lg transition-colors">
            <Trash2 size={16} />
          </button>
        )}
        {!mission.completed ? (
